...
public class SingleModuleCallGraphExtractor {
    ...
    /** EagleConstant: 绑定缺失时的兜底键，类名 + # + 字段名 -> 解析后的 URL/常量值。 */
    private static final Map<String, String> eagleConstantFieldNameToResolvedValue = new HashMap<>();
    ...
    private static final String UPSERT_BACKEND_SERVICE_DEFINE_SQL = """
            INSERT INTO backend_service_define (project_name, zhisheng_interface, service_url, comment, controller_method, zhisheng_comment, is_manual_defined)
            VALUES (?, ?, ?, ?, ?, ?, 0)
            ...
            WHERE COALESCE(backend_service_define.is_manual_defined, 0) = 0
            """;
                    ...
                    for (Object o : node.fragments()) {
                        ...
                        if (classBinding != null && TARGET_CLASS_FQN.equals(qn) && vb != null) {
                            ...
                            if (TARGET_CONSTANT_PREFIXES.stream().anyMatch(fname::startsWith)) {
                                String key = resolveEagleConstantValue(
                                        vb, classBinding, typeNode, frag, cu, fileMemo, new HashSet<>());
                                if (isResolvedConstantKey(key)) {
                                    eagleConstantFieldKeyToResolvedValue.put(vb.getVariableDeclaration().getKey(), key);
                                    String fallbackKey = buildEagleConstantFallbackKey(classBinding.getQualifiedName(), fname);
                                    if (fallbackKey != null) {
                                        eagleConstantFieldNameToResolvedValue.put(fallbackKey, key);
                                    }
...
//                                    String constantFqn = declaringClass.getQualifiedName() + "." + fieldName;
    ...
    /** 手工录入行 is_manual_defined=1，分析任务只应覆盖自动行。 */
    private static void migrateIsManualDefinedColumn(Connection conn) throws SQLException {
        if (tableHasColumn(conn, "backend_service_define", "is_manual_defined")) {
            return;
        }
        try (Statement st = conn.createStatement()) {
            st.execute("ALTER TABLE backend_service_define ADD COLUMN is_manual_defined INTEGER NOT NULL DEFAULT 0");
        }
    }
    ...
    private static final String DELETE_BACKEND_SERVICE_BY_PROJECT_SQL =
            "DELETE FROM backend_service_define WHERE project_name = ? AND COALESCE(is_manual_defined, 0) = 0";
